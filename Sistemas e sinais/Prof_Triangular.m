...
Dn = eval(Dn);
D_o = eval(D_o);      
% Corrigindo o valor médio (NaN devido a indeterminação)
Dn(N+1) = D_o;        
% Subistituindo no vetor de respostas
%% Visualizando o espector de Amplitude 
figure(1) 
stem(f,abs(Dn),"linewidth", 3);                          
% Na frequência o sinal é discretotitle('Serie de Fourier do sinal g(t)');
...
% o pi é necessário pois está incorporado         
...
figure(4) 
stem(f,abs(Dn),"linewidth", 3);                          
% Na frequência o sinal é discreto
